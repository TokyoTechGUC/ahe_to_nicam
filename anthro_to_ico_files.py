...
glev = 5
rlev = 0
hgrid_base = '/gs/hs1/jh230052/tutorial_package/NICAM_DATABASE/hgrid/gl05/rl00/grid'
vgrid_base = '/gs/hs1/jh230052/tutorial_package/NICAM_DATABASE/vgrid/vgrid38.dat'
ll_imax = 144 #Confirm input netcdf
ll_jmax = 73 #Confirm input netcdf
lon_ini = 0.0
lat_ini = -90.0
lon_inc = 2.5
lat_inc = 2.5
...
def run_grd2ico(infold,outfold):
    filelist = glob(f'{infold}/*.bin')
    for ifil in filelist:
        generate_grd2icocnf(ifil,os.path.basename(ifil.replace('.bin','').replace('_2.5deg','')))
        subprocess.run(['./grd2ico'])
        if runico2ll:
            runico2ll(os.path.basename(ifil.replace('_2.5deg.bin','')))
